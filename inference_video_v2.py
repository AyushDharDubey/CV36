...
INPUT_VIDEO    = r"D:\videos\VBOX0002_0001.mp4"
...
if __name__ == '__main__':
    ...
    def suppress_rider_persons(all_dets):
        ...
        1. Remove 'person' boxes that overlap with any vehicle box (riders)
        2. Remove COCO vehicle boxes (truck/car/motorcycle) that overlap with
           an E-rickshaw box — prevents double-detection of e-rickshaws
        """
        erick_boxes = [
            (d['x1'], d['y1'], d['x2'], d['y2'])
            for d in all_dets if d['label'] == 'E-rickshaw'
        ]

        vehicle_boxes = [
            (d['x1'], d['y1'], d['x2'], d['y2'])
            for d in all_dets if d['label'] in VEHICLE_CLASSES
        ]
        ...
        for det in all_dets:
            ...
            # Rule 1 — suppress person if overlapping a vehicle (rider)
            if label == 'person':
                is_rider = any(
                    compute_iou(box, vbox) >= PERSON_VEHICLE_IOU_THRESHOLD
            ...
            # Rule 2 — suppress COCO vehicle if overlapping an e-rickshaw box
            # (prevents truck/car/motorcycle double-labelling of e-rickshaws)
            if label in {'truck', 'car', 'motorcycle', 'bus', 'bicycle'}:
                overlaps_erick = any(
                    compute_iou(box, ebox) >= PERSON_VEHICLE_IOU_THRESHOLD
